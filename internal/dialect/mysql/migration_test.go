...
func TestGenerateIndexCreatesModifiedEmptyName(t *testing.T) {
	g := NewMySQLGenerator()
	...
	td := &diff.TableDiff{
		...
		ModifiedIndexes: []*diff.IndexChange{
			{
				Old: &core.Index{Name: "idx_old", Columns: []core.IndexColumn{{Name: "email"}}},
	...
	require.Len(t, result.Statements, 1)
	assert.Equal(t, "", result.Statements[0])
	assert.Empty(t, result.Rollback)
}

func TestGenerateIndexCreatesAddedEmptyName(t *testing.T) {
	g := NewMySQLGenerator()
